...
export function WorldChapterSections({
  ...
  const contactLinks = [
    { label: "Email", href: `mailto:${personInfo.email}`, value: personInfo.email },
    { label: "LinkedIn", href: personInfo.linkedIn!, value: "Connect on LinkedIn" },
            ...
            >
              ...
              <div className="mb-4">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between sm:gap-4">
                  <h3 className="font-serif text-2xl text-foreground">{entry.title}</h3>
      ...
      >
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {featuredProjects.map((project) => (
            <article
              key={project.id}
              className="group relative flex h-full flex-col rounded-lg border border-border/50 bg-background p-6 transition-all duration-300 hover:border-accent-cta/50 hover:bg-muted/30 hover:shadow-md"
            >
              <h3 className="mb-4 font-serif text-xl text-foreground transition-colors group-hover:[color:var(--color-accent-cta)]">
                {project.title}
              </h3>
              <p className="mb-4 grow text-muted-foreground">{project.description}</p>
